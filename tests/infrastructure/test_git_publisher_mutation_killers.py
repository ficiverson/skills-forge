...
class TestGitRegistryPublisherInitMutationKillers:
    ...
    def test_init_custom_git_command_usage(self, registry_root: Path, tmp_path: Path, manifest: SkillPackManifest) -> None:
        ...
        with patch("subprocess.run") as mock_run, patch("shutil.copyfile"), \
             patch("skill_forge.infrastructure.adapters.git_registry_publisher._sha256", return_value="a"*64):
...
class TestGitRegistryPublisherMutationKillers:
    def test_publish_exact_git_commands(
        self, publisher: GitRegistryPublisher, registry_root: Path, manifest: SkillPackManifest, tmp_path: Path
    ) -> None:
        # Create a dummy pack
        pack_path = tmp_path / "test.skillpack"
        ...
        with patch("subprocess.run") as mock_run, \
             ...
             patch("skill_forge.infrastructure.adapters.git_registry_publisher._sha256", return_value="a" * 64), \
             patch("skill_forge.infrastructure.adapters.git_registry_publisher._now_iso", return_value="2026-04-15T12:00:00Z"):
            ...
            # Check git add
            assert ["git", "-C", str(registry_root), "add", "packs/dev/my-skill-1.0.0.skillpack", "index.json"] in calls
            
            # Check git diff (has_staged_changes)
            assert ["git", "-C", str(registry_root), "diff", "--cached", "--name-only"] in calls
            
            # Check git commit
            assert ["git", "-C", str(registry_root), "commit", "-m", "Custom commit message"] in calls
    ...
    def test_publish_default_commit_message(
        ...
        with patch("subprocess.run") as mock_run, \
            ...
            assert ["git", "-C", str(registry_root), "commit", "-m", "Publish my-skill 1.0.0"] in calls
